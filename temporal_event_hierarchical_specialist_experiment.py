...
OUT=Path('temporal_event_hierarchical_specialist_results'); OUT.mkdir(exist_ok=True)
...
def evaluate(d,features,threshold):
    ...
    for year in sorted(ev.fyDeclared.astype(int).unique()):
        idx=np.where(ev.fyDeclared.astype(int).to_numpy()==year)[0]; te=ev.iloc[idx]; low=te.target<=300e6; upper=~low; pp=np.zeros(len(te))
        if low.any(): pp[np.where(low.to_numpy())[0]]=.5*lower_direct(d,features,te.loc[low],year,9000+year)+.5*lower_position(d,features,te.loc[low],year,9000+year)
        if upper.any():
            ...
            for j,(_,r) in zip(np.where(upper.to_numpy())[0],te.loc[upper].iterrows()):
                ...
                if same>=threshold:
                    p=float(seen_family_upper(d,te.loc[[r.name]],year,9000+year)[0]); method='seen_family_static'; detail={'same_family_support':same}
                else:
                    p,detail=unseen_event_upper(d,e,r,year); detail['same_family_support']=same; method='unseen_family_event_fallback'
...
def main():
    ...
    for t in [2,3,4]:
        ...
        if t==3: chosen=(out,m)
    out,m=chosen; upper=out[out.target>300e6][['disasterNumber','state','fyDeclared','incidentType','target','prediction','method','details']]; upper.to_csv(OUT/'upper_300_500M_strict_predictions.csv',index=False)
    summary={'family_support_threshold':3,'threshold_sensitivity':sens,'selected_threshold_3_metrics':m,'counts':{'200_300M':int((out.target<=300e6).sum()),'300_500M':int((out.target>300e6).sum())},'notes':['Every supervised funding model excludes all rows from the held-out fiscal year.','For upper cases with at least three >100M <=1B support cases of the same incident family outside the held year, a static CatBoost expert is used.','For an unseen high-value incident family, an exact-event Ridge model predicts total event funding from target-free event aggregates and a separately trained mission-share model allocates the predicted event total to the state.','No held-year funding label enters the event-total or share model.','The exact-event fallback assumes target-free peer response descriptors for the same event are available; it is not strictly declaration-time causal.','The true funding band still selects the 200-300M versus 300-500M evaluation specialist, so this remains oracle-band specialist validation rather than deployable router performance.','The architecture was developed on this dataset and the upper subgroup has only four cases; freeze it before final untouched evaluation.']}
