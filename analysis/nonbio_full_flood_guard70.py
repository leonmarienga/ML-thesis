...
OUT = ROOT / "audit_outputs" / "nonbio_full_flood_guard70"
...
def main():
    ...
    for fy in sorted(nonbio["fyDeclared"].astype(int).unique()):
        ...
        # Strict nested guard70 Flood $1M rescue threshold.
        # Threshold is learned only from outer-training years.
        flood_inner = inner_base_and_flood_scores(
            train[train["target_clean"] < 50_000_000].copy(),
            semfeat,
            flood_features,
            int(fy),
        )
        flood_th, flood_diag = choose_rescue_threshold(
            flood_inner, 0.70
        )
    ...
    md = [
        "# Full-router Flood guard70 integration audit",
        "",
        "- Baseline: accepted 812/912 router.",
        "- Candidate adds only the strict one-way Flood guard70 rescue to the accepted low branch.",
        "- Flood rescue threshold is learned only from outer-training nested LFYO predictions and preserves at least 70% Flood $100K-$1M recall in inner data.",
        "",
