#!/usr/bin/env python3
"""
Deterministic strict-LFYO confirmation of the Hurricane-specific >=$50M veto.

Both variants share the exact same data, folds, root, global sentinel,
low-boundary model and high hierarchy. The only difference is whether the
Hurricane-only one-way veto is enabled.
...
OUT = ROOT / "audit_outputs" / "nonbio_hurricane_boundary_confirm"
OUT.mkdir(parents=True, exist_ok=True)


def data_hash(ma: pd.DataFrame) -> str:
    cols = [c for c in [
        "disasterNumber", "maId", "maAmendNumber", "agencyId", "supportFunction",
        "maType", "priority", "assistanceRequested", "statementOfWork"
    ] if c in ma.columns]
    x = ma[cols].copy().fillna("").astype(str).sort_values(cols).reset_index(drop=True)
    return hashlib.sha256(x.to_csv(index=False).encode("utf-8")).hexdigest()


def fit_stage1(train, features, seed):
    t = train[train["target_clean"] < 50_000_000].copy()
    y = (t["target_clean"] >= 100_000).astype(int)
...
def build_low_predictor(train, semfeat, outer_fy):
    oof = inner_binary_oof(
        train, semfeat, stage=1, kind="rf", seedbase=110000
    )
    threshold, _ = choose_threshold(oof, "macro_f1")
    ...
    def predict(dd):
        ...
        p1 = s1.predict_proba(
            normalize_model_frame(dd[semfeat])
        )[:, 1]
        out = np.full(len(dd), "0-100K", dtype=object)
        idx = np.flatnonzero(p1 >= threshold)
        if len(idx):
            dollars = np.expm1(
                reg.predict(normalize_model_frame(dd.iloc[idx][semfeat]))
            )
            out[idx] = np.where(
                dollars >= 1_000_000,
                "1M-50M",
...
def fit_hurricane_binary(train, features, seed):
...
def hurricane_oof(outer_train, features):
    d = outer_train[
    ...
    for fy in sorted(d["fyDeclared"].astype(int).unique()):
        ...
        if te.empty or ytr.nunique() < 2:
            continue
        model = fit_hurricane_binary(tr, features, 400000 + fy)
...
def main():
    ...
    variants = ["baseline", "hurricane_log"]
    ...
    for fy in sorted(nonbio["fyDeclared"].astype(int).unique()):
        ...
        # Hurricane-only verifier.
        ho = hurricane_oof(train, semext)
        hth, hdiag = safe_threshold(ho)
        htrain = train[
            (train["incidentType"] == "Hurricane")
            & (train["target_clean"] >= 1_000_000)
        ].copy()
        hy = (
            htrain["target_clean"] >= 50_000_000
        ).astype(int)
        hmodel = None
        if len(htrain) >= 6 and hy.nunique() >= 2:
            hmodel = fit_hurricane_binary(
                htrain,
                semext,
                420000 + fy,
            )
        ...
        diagnostics.append({
            ...
            "inner_hurricane_recall": hdiag.get("recall"),
            "inner_hurricane_precision": hdiag.get("precision"),
            "inner_hurricane_fp": hdiag.get("fp"),
        })

        roots = {
            "baseline": global_root.copy(),
            "hurricane_log": global_root.copy(),
        }

        idx = np.flatnonzero(
            (roots["hurricane_log"] == 1)
            & (
                test["incidentType"].to_numpy()
                == "Hurricane"
            )
        )
        if len(idx) and hmodel is not None:
            hp = hmodel.predict_proba(
                normalize_model_frame(
                    test.iloc[idx][semext]
                )
            )[:, 1]
            roots["hurricane_log"][
                idx[hp < hth]
        ...
        for variant in variants:
            ...
            low_map = {
                int(dn): str(p)
                for dn, p in zip(
                    pred_low_rows["disasterNumber"], lp
                )
            }
    ...
    for variant in variants:
        ...
        p.to_csv(
            OUT / f"{variant}_predictions.csv",
            index=False,
        )
        ...
        results[variant] = {
            ...
            "end_to_end": six_band_metrics(
                p, "final_pred"
            ),
            "high_false_negatives": p.loc[
    ...
    baseline_fn = {
        x["disasterNumber"]
        for x in results["baseline"][
            "high_false_negatives"
        ]
    }
    hurricane_fn = {
        x["disasterNumber"]
        for x in results["hurricane_log"][
            "high_false_negatives"
        ]
    }
    assert hurricane_fn == baseline_fn, (
        f"Hurricane veto changed high FN set: "
        f"{baseline_fn} -> {hurricane_fn}"
    )
    ...
    summary = {
        "mission_nonfinancial_sha256": ma_snapshot_hash,
        "results": results,
    }
    (
        OUT / "summary.json"
    ...
    md = [
        "# Hurricane boundary confirmation",
        "",
        "- Same-run deterministic baseline and Hurricane-log veto.",
        "- Safety assertion: high-value false-negative set is unchanged.",
        "- Mission nonfinancial snapshot SHA256: " + ma_snapshot_hash,
        "",
        "| Variant | High recall | Precision | Root FP | Overall | Macro | 100K-1M | 1M-50M | 50-200M | 200-500M | 500M+ |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
