#!/usr/bin/env python3
"""
Strict-LFYO two-key Hurricane veto audit.
...
Candidate second veto:
1. operational-depth Random Forest says LOW
AND
2. an independent DFA-mechanism verifier says LOW

DFA-mechanism features are deliberately narrow and interpretable:
- initial_usace_esf3_dfa_count
- sem_type_dfa_count
- sem_type_dfashare
- sem_type_fosshare

DFA verifier variants:
- logistic
- random forest
- both must agree (triple consensus with depth RF)

Every component threshold is selected only from inner-LFYO outer-training
predictions and individually calibrated to preserve all true >=$50M Hurricane
training cases. Any variant that changes the baseline high-value FN set is
...
OUT = ROOT / "audit_outputs" / "nonbio_hurricane_two_key_veto"
...
def fit_dfa_binary(train: pd.DataFrame, features, kind: str, seed: int):
...
def dfa_oof(outer_train: pd.DataFrame, features, kind: str, seedbase: int):
    d = outer_train[
    ...
    for fy in sorted(d["fyDeclared"].astype(int).unique()):
        ...
        model = fit_dfa_binary(
            tr, features, kind, seedbase + fy
        )
...
def safe_threshold(oof: pd.DataFrame):
...
def main():
    ...
    df = (
        ...
        .merge(
            ext, on="disasterNumber", how="left"
        )
        .merge(
            mech, on="disasterNumber", how="left"
        )
        .merge(
            eag, on="disasterNumber", how="left"
        )
    )
    ...
    missing_depth = [
        c for c in DEPTH_FEATURES
        if c not in nonbio.columns
    ]
    missing_dfa = [
        c for c in DFA_FEATURES
        if c not in nonbio.columns
    ]
    if missing_depth or missing_dfa:
        raise RuntimeError(
            f"Missing features depth={missing_depth}, dfa={missing_dfa}"
        )

    depth_features = list(DEPTH_FEATURES)
    dfa_features = list(DFA_FEATURES)
    ...
    variants = [
        "binary_only",
        "depth_rf_unsafe_reference",
        "depth_rf_and_dfa_log",
        "depth_rf_and_dfa_rf",
        "depth_rf_and_both_dfa",
    ]
    ...
    for fy in sorted(
        ...
        # Independent DFA mechanism verifiers.
        dfa_vetoes = {}
        ...
        for kind in ["log", "rf"]:
            ...
            dfa_vetoes[kind] = veto

            diagnostics.append({
                "outer_fy": int(fy),
                "dfa_kind": kind,
                "depth_threshold": float(dth),
                "depth_inner_recall": (
                    ddiag.get("recall")
                ),
                "dfa_threshold": float(th),
                "dfa_inner_recall": (
                    dg.get("recall")
                ),
                "dfa_inner_precision": (
        ...
        roots = {
            "binary_only": binary_root.copy(),
            "depth_rf_unsafe_reference": binary_root.copy(),
            "depth_rf_and_dfa_log": binary_root.copy(),
            "depth_rf_and_dfa_rf": binary_root.copy(),
            "depth_rf_and_both_dfa": binary_root.copy(),
        }
    ...
    summary = {
        ...
        "results": results,
        "development_note": (
            "The DFA feature family was motivated by prior exploratory "
            "mechanism comparison. This remains development validation."
        ),
    }
    ...
    md = [
        "# Hurricane two-key veto audit",
        "",
        "- Baseline: confirmed binary Hurricane semext veto.",
        "- Candidate veto requires operational-depth RF LOW plus independent DFA-mechanism LOW.",
        "- Every threshold selected by inner LFYO.",
        "",
