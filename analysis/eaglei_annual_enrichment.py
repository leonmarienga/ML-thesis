#!/usr/bin/env python3
"""
Target-blind annual EAGLE-I outage enrichment.
...
All FEMA disasters beginning in calendar 2022 are enriched BEFORE funding is
...
Annual EAGLE-I file is selected from the official Figshare release.
Modeled county customer counts (MCC.csv) file id: 42547708

Annual outage files are large, so they are streamed in pandas chunks.
"""
...
YEAR = int(os.environ.get("EAGLE_YEAR", "2022"))
if YEAR not in EAGLE_FILE_IDS:
    raise ValueError(f"Unsupported EAGLE_YEAR={YEAR}; choose one of {sorted(EAGLE_FILE_IDS)}")
EAGLE_FILE_ID = EAGLE_FILE_IDS[YEAR]
EAGLE_URL = f"https://ndownloader.figshare.com/files/{EAGLE_FILE_ID}"
MCC_URL = "https://ndownloader.figshare.com/files/42547708"

OUT = ROOT / "audit_outputs" / "eaglei_annual" / str(YEAR)
CACHE = ROOT / ".cache" / "eaglei_annual" / str(YEAR)
OUT.mkdir(parents=True, exist_ok=True)
CACHE.mkdir(parents=True, exist_ok=True)

EAGLE_PATH = CACHE / f"eaglei_outages_{YEAR}.csv"
...
def build_candidate_windows(master: pd.DataFrame) -> pd.DataFrame:
    ...
    x = x[x["incidentBegin_dt"].dt.year == YEAR].copy()
...
def main():
    ...
    print(f"Calendar-{YEAR} FEMA candidates: {len(candidates)}")
    ...
    features.to_csv(OUT/f"eaglei_features_all_{YEAR}_fema.csv",index=False)
    timeline.to_csv(OUT/f"eaglei_timelines_all_{YEAR}_fema.csv",index=False)
    view.to_csv(OUT/f"eaglei_{YEAR}_fema_diagnostic.csv",index=False)

    key = view[pd.to_numeric(view["totalObligatedFunding"], errors="coerce") >= 50_000_000].copy()
    key.to_csv(OUT/f"high_value_{YEAR}_cases.csv",index=False)
